...
const BatchProductManagement = () => {
  ...
  const handleDownloadAllQR = async (batchId, batchNumber) => {
    try {
      const token = localStorage.getItem("authToken")
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL || "http://localhost:8080/api"}/batches/${batchId}/items/qr-codes`,
        {
          headers: token ? { Authorization: `Bearer ${token}` } : {},
          responseType: "blob",
        },
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const token = localStorage.getItem("authToken")
      await axios.delete(
        `${process.env.REACT_APP_API_URL || "http://localhost:8080/api"}/batches/${batchToDelete.id}`,
        {
          headers: token ? { Authorization: `Bearer ${token}` } : {},
        },
      )
                ...
                <div
                  ...
                  <div className="batch-actions">
                    ...
                    <button
                      ...
                      onClick={() =>
                        handleDownloadAllQR(batch.id, batch.batchNumber)
                      }
                      disabled={!batch.itemsCount || batch.itemsCount === 0}>
                      <Download size={16} />
                      Tải QR
